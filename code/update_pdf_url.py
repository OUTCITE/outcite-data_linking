...
def get_url_for(refobject,field,id_field,cur,USE_BUFFER): #TODO: If I want to store the resolved urls then I need to return the results and store them outside this function!
    ...
    if id_field=='ssoar_id' and 'ssoar_id' in refobject and refobject['ssoar_id'] and (_retest or not (_to_field[:-1] in refobject and refobject[_to_field[:-1]])):
        ...
        resolution = (None,None,None,) if not _check else (url,418,url,) if not url_ else (url,200,url_,);
        if url_:
        ...
        return ids, refobject, resolution;
...
def get_url(refobjects,field,id_field,cur=None,USE_BUFFER=None): # This actually gets the doi not the url 
    ...
    for i in range(len(refobjects)):
        if _workers>1:
            pool.apply_async(get_url_for, args=(refobjects[i],field,id_field,None,'r' if (USE_BUFFER=='rw' or USE_BUFFER=='r') else None,), callback=append_result);
        else:
            _temp_parallel_results.append(get_url_for(refobjects[i],field,id_field,cur,USE_BUFFER));
    if _workers>1:
        pool.close(); pool.join();
    print(_temp_parallel_results);
    for ids_,refobject,resolution in _temp_parallel_results:
        ids          += ids_;
